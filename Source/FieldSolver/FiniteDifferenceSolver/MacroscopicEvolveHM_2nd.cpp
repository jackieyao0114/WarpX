        ...
        amrex::ParallelFor(tbx, tby, tbz,
            [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                ...
                if (mag_Ms_arrx(i, j, k) != 0 && mag_alpha_arrx(i, j, k) != 0 && mag_gamma_arrx(i, j, k) != 0){
                    ...
                    Real a_temp_static_coeff = mag_alpha_arrx(i, j, k) / M_magnitude;
            ...
            [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                ...
                if (mag_Ms_arry(i, j, k) != 0 && mag_alpha_arry(i, j, k) != 0 && mag_gamma_arry(i, j, k) != 0){
                    ...
                    Real M_magnitude = (M_normalization == 0) ? std::sqrt(std::pow(M_yface(i, j, k, 0), 2.0) + std::pow(M_yface(i, j, k, 1), 2.0) + std::pow(M_yface(i, j, k, 2), 2.0))
                                                              : mag_Ms_arry(i, j, k);
                    Real a_temp_static_coeff = mag_alpha_arry(i, j, k) / M_magnitude;
            ...
            [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                ...
                if (mag_Ms_arrz(i, j, k) != 0 && mag_alpha_arrz(i, j, k) != 0 && mag_gamma_arrz(i, j, k) != 0){
                    ...
                    Real M_magnitude = (M_normalization == 0) ? std::sqrt(std::pow(M_zface(i, j, k, 0), 2.0) + std::pow(M_zface(i, j, k, 1), 2.0) + std::pow(M_zface(i, j, k, 2), 2.0))
                                                              : mag_Ms_arrz(i, j, k);
                    Real a_temp_static_coeff = mag_alpha_arrz(i, j, k) / M_magnitude;
                ...
                [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                    ...
                    if (mag_Ms_arrx(i, j, k) != 0 && mag_alpha_arrx(i, j, k) != 0 && mag_gamma_arrx(i, j, k) != 0){
                        ...
                        a_temp_xface(i, j, k, 0) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hx_eff + a_temp_static_xface(i, j, k, 0))
                                                                          : -(dt * a_temp_dynamic_coeff * Hx_eff + 0.5 * a_temp_static_xface(i, j, k, 0) 
                                                                            + 0.5 * mag_alpha_arrx(i, j, k) * 1. / std::sqrt(std::pow(M_xface(i, j, k, 0), 2.0) + std::pow(M_xface(i, j, k, 1), 2.0) + std::pow(M_xface(i, j, k, 2), 2.0)) * M_old_xface(i, j, k, 0));

                        // y component on x-faces of grid
                        a_temp_xface(i, j, k, 1) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hy_eff + a_temp_static_xface(i, j, k, 1))
                                                                          : -(dt * a_temp_dynamic_coeff * Hy_eff + 0.5 * a_temp_static_xface(i, j, k, 1) 
                                                                            + 0.5 * mag_alpha_arrx(i, j, k) * 1. / std::sqrt(std::pow(M_xface(i, j, k, 0), 2.0) + std::pow(M_xface(i, j, k, 1), 2.0) + std::pow(M_xface(i, j, k, 2), 2.0)) * M_old_xface(i, j, k, 1));

                        // z component on x-faces of grid
                        a_temp_xface(i, j, k, 2) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hz_eff + a_temp_static_xface(i, j, k, 2))
                                                                          : -(dt * a_temp_dynamic_coeff * Hz_eff + 0.5 * a_temp_static_xface(i, j, k, 2) 
                                                                            + 0.5 * mag_alpha_arrx(i, j, k) * 1. / std::sqrt(std::pow(M_xface(i, j, k, 0), 2.0) + std::pow(M_xface(i, j, k, 1), 2.0) + std::pow(M_xface(i, j, k, 2), 2.0)) * M_old_xface(i, j, k, 2));
                    ...
                    if (mag_Ms_arry(i, j, k) != 0 && mag_alpha_arry(i, j, k) != 0 && mag_gamma_arry(i, j, k) != 0){
                        ...
                        a_temp_yface(i, j, k, 0) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hx_eff + a_temp_static_yface(i, j, k, 0))
                                                                          : -(dt * a_temp_dynamic_coeff * Hx_eff + 0.5 * a_temp_static_yface(i, j, k, 0) 
                                                                            + 0.5 * mag_alpha_arry(i, j, k) * 1. / std::sqrt(std::pow(M_yface(i, j, k, 0), 2.0) + std::pow(M_yface(i, j, k, 1), 2.0) + std::pow(M_yface(i, j, k, 2), 2.0)) * M_old_yface(i, j, k, 0));
                        ...
                        a_temp_yface(i, j, k, 1) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hy_eff + a_temp_static_yface(i, j, k, 1))
                                                                          : -(dt * a_temp_dynamic_coeff * Hy_eff + 0.5 * a_temp_static_yface(i, j, k, 1) 
                                                                            + 0.5 * mag_alpha_arry(i, j, k) * 1. / std::sqrt(std::pow(M_yface(i, j, k, 0), 2.0) + std::pow(M_yface(i, j, k, 1), 2.0) + std::pow(M_yface(i, j, k, 2), 2.0)) * M_old_yface(i, j, k, 1));
                        ...
                        a_temp_yface(i, j, k, 2) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hz_eff + a_temp_static_yface(i, j, k, 2))
                                                                          : -(dt * a_temp_dynamic_coeff * Hz_eff + 0.5 * a_temp_static_yface(i, j, k, 2) 
                                                                            + 0.5 * mag_alpha_arry(i, j, k) * 1. / std::sqrt(std::pow(M_yface(i, j, k, 0), 2.0) + std::pow(M_yface(i, j, k, 1), 2.0) + std::pow(M_yface(i, j, k, 2), 2.0)) * M_old_yface(i, j, k, 2));
                    ...
                    if (mag_Ms_arrz(i, j, k) != 0 && mag_alpha_arrz(i, j, k) != 0 && mag_gamma_arrz(i, j, k) != 0){
                        ...
                        a_temp_zface(i, j, k, 0) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hx_eff + a_temp_static_zface(i, j, k, 0))
                                                                          : -(dt * a_temp_dynamic_coeff * Hx_eff + 0.5 * a_temp_static_zface(i, j, k, 0) 
                                                                            + 0.5 * mag_alpha_arrz(i, j, k) * 1. / std::sqrt(std::pow(M_zface(i, j, k, 0), 2.0) + std::pow(M_zface(i, j, k, 1), 2.0) + std::pow(M_zface(i, j, k, 2), 2.0)) * M_old_zface(i, j, k, 0));
                        ...
                        a_temp_zface(i, j, k, 1) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hy_eff + a_temp_static_zface(i, j, k, 1))
                                                                          : -(dt * a_temp_dynamic_coeff * Hy_eff + 0.5 * a_temp_static_zface(i, j, k, 1) 
                                                                            + 0.5 * mag_alpha_arrz(i, j, k) * 1. / std::sqrt(std::pow(M_zface(i, j, k, 0), 2.0) + std::pow(M_zface(i, j, k, 1), 2.0) + std::pow(M_zface(i, j, k, 2), 2.0)) * M_old_zface(i, j, k, 1));
                        ...
                        a_temp_zface(i, j, k, 2) = (M_normalization != 0) ? -(dt * a_temp_dynamic_coeff * Hz_eff + a_temp_static_zface(i, j, k, 2))
                                                                          : -(dt * a_temp_dynamic_coeff * Hz_eff + 0.5 * a_temp_static_zface(i, j, k, 2) 
                                                                            + 0.5 * mag_alpha_arrz(i, j, k) * 1. / std::sqrt(std::pow(M_zface(i, j, k, 0), 2.0) + std::pow(M_zface(i, j, k, 1), 2.0) + std::pow(M_zface(i, j, k, 2), 2.0)) * M_old_zface(i, j, k, 2));
